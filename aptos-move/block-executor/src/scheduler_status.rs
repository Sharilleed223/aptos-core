...
Executing(i) ------------------------------> Executed(i)
    |                                           |
    | try_abort(i) + finish_abort(i)            | try_abort(i) + finish_abort(i)
    |                                           |
    ↓                    finish_execution       ↓
...
requires the caller to eventually perform a corresponding `remove_stall`.
...
pub(crate) struct ExecutionStatus {
    ...
    ///
    /// This counter is monotonically increasing and updated in a successful try_abort.
    ...
    next_incarnation_to_try_abort: CachePadded<AtomicU32>,
    ...
    execution_queue_manager: Arc<ExecutionQueueManager>,
...
impl ExecutionStatus {
    pub(crate) fn new(
        ...
        Self {
            ...
            next_incarnation_to_try_abort: CachePadded::new(AtomicU32::new(0)),
    ...
    pub(crate) fn try_start_executing(&self) -> Result<Option<Incarnation>, PanicError> {
        ...
        if ret.is_some() {
            ...
            // It could also be SAFE if the transaction was previously executed
            // and is now being re-executed.
    ...
    /// Completes the abortion of a transaction incarnation.
    ///
    /// This is the second step of the two-step abort process. It must be called
    /// after a successful try_abort and updates the transaction's status:
    ...
    ///
    /// # Note
    /// Each successful add_stall must be balanced by a corresponding remove_stall call.
...
impl ExecutionStatus {
    ...
    // only asserting that new incarnation > previously stored abort shortcut.
    fn incarnate(
        &self,
...
impl ExecutionStatus {
    ...
    pub(crate) fn new_for_test(
        ...
        num_stalls: u32,
        manager: &Arc<ExecutionQueueManager>,
        txn_idx: TxnIndex,
    ) -> Self {
        ...
        Self {
            ...
            next_incarnation_to_try_abort: CachePadded::new(AtomicU32::new(incarnation)),
            dependency_shortcut: CachePadded::new(AtomicU8::new(shortcut)),
            num_stalls: CachePadded::new(AtomicU32::new(num_stalls)),
            execution_queue_manager: manager.clone(),
            txn_idx,
        }
    }
...
mod tests {
    ...
    fn status_cycle_with_finish_and_resolve(stall_before_finish: bool) {
        let txn_idx = 50;
        let manager = Arc::new(ExecutionQueueManager::new_for_test(txn_idx));
        let status = ExecutionStatus::new(manager.clone(), txn_idx);
    ...
    fn stall_executed_status() {
        let manager = Arc::new(ExecutionQueueManager::new_for_test(10));
    ...
    fn set_executing_flag_err(case: bool) {
        let status = ExecutionStatus::new(Arc::new(ExecutionQueueManager::new_for_test(10)), 10);
